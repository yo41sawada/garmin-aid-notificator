...
|------|------|
| 補給間隔 | 5km 固定 |
| 距離の表示精度 | 小数点1桁（例: `3.8km`） |
| 初期表示 | 5.0km（アクティビティ開始前も表示） |
...
**Settings → Tones / Vibrate** のチェック状態を切り替えることで、デバイス設定のオン/オフをシミュレートできます。補給ポイント通過時の動作は以下の4パターンで確認してください。

| Tones | Vibrate | 期待する動作 |
|-------|---------|------------|
| ✅ ON | ✅ ON | トーン鳴動 ＋ バイブあり |
| ✅ ON | ☐ OFF | トーン鳴動のみ |
| ☐ OFF | ✅ ON | バイブのみ |
